...
# Hard replace every existing /register rule. Do not stack handlers.
for rule in list(app.url_map.iter_rules()):
    if rule.rule == '/register':
        try:
...
@app.route('/register', methods=['GET', 'POST'], endpoint='register_team')
def register_team():
    db = get_db()
    ...
    if request.method == 'POST':
        ...
        if not name or not contact or team_number not in TEAM_SLOTS:
            ...
            try:
                ...
                response = make_response(render_template(
                    'register.html', success=True, temp_id=temp_id,
                    team_number=team_number, available_teams=[]
                ))
                response.set_cookie(
                    TEAM_COOKIE, raw_token,
                    max_age=48 * 60 * 60,
                    httponly=True,
                    samesite='Lax',
